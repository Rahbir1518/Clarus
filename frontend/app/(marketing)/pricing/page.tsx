...
function PricingContent() {
  ...
  // After sign-in redirect: if ?plan=<tier> is in the URL and the visitor is
  ...
  const handleGetStarted = useCallback(
    (tierKey: string) => {
      ...
      // Clerk carries the chosen plan back through the redirect URL, which is
      // where the effect above picks it up.
      const returnTo = encodeURIComponent(`/pricing?plan=${tierKey}`);
      window.location.href = `/signIn?redirect_url=${returnTo}`;
    },
    [isSignedIn],
  );
